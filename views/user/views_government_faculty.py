...
from common.utils import dictionary_faculty_key
from kbds.user.faculty_kbd import FacultyCbData
from kbds.user.main_kbd import main_keyboard

view_gov_faculty_router = Router()

@view_gov_faculty_router.callback_query(FacultyCbData.filter(F.previous_action == "faculties_gov"))
async def show_student_faculty_government(callback_query: CallbackQuery, session: AsyncSession):
    _, faculty, _ = callback_query.data.split(":")
    structure = str(await dictionary_faculty_key(faculty))
    datas = await orm_get_positions(session, structure)
    if not datas:
        await callback_query.answer("Дані відсутні.", show_alert=True)
        return

    await send_faculty_government_page(callback_query, session, datas, structure, index=0)


async def send_faculty_government_page(callback_query: CallbackQuery, session: AsyncSession, datas, structure, index: int):
    data = datas[index]
...
@view_gov_faculty_router.callback_query(F.data.contains("_faculty-gov_"))
async def paginate_government(callback_query: CallbackQuery, session: AsyncSession):
    ...
    await send_faculty_government_page(callback_query, session, datas, structure, index)
